...
(function ($) {
    var UsersSelectionPlugin = function () {
    };
     ...
     * @returns {undefined}
     */
    UsersSelectionPlugin.prototype.setMemberListPlugin = function (plugin, pluginInitFunc) {
    ...
    UsersSelectionPlugin.prototype.setAddByIdPlugin = function (plugin, pluginInitFunc) {
    ...
    UsersSelectionPlugin.prototype.init = function (params) {
        $.extend(true, std_obj, params);
    };

    UsersSelectionPlugin.prototype.setUserPlugin = function (plugin) {
        std_obj.plugins.user = plugin;
    };


    UsersSelectionPlugin.prototype.bind = function (params, container) {
    ...
    var std_obj = {
        ...
        locale: {
            ...
            inviteGuests: "Добавить гостей",
            invited: "Приглашенные участники",
            addNewParticipants: "Добавить новых участников",
            ok: "Применить",
            save: "Сохранить",
            saving: "Сохранение",
            cancel: "Отмена",
            addFromList: "Выбрать из списка",
            selectFromGroup: "Выбрать из группы",
            otherAddingMethods: "Другие способы добавления",
            unknownError: "Неизвестная ошибка",
            systemError: "Системная ошибка",

            titleInviteMessage: "Пригласительное сообщение",
            titleInviteMessageSubject: "Тема пригласительного сообщения",
            inviteMessage: "Текст приглашения",

            pleaseWait: "Пожалуйста, подождите",
            newParticipantsMessage: "Новым участникам будут отправлены приглашения на почту",
            insertUserId: 'Введите id пользователя',
            add: 'Добавить',
            userAlreadyAdded: 'Пользователь уже есть в списке',
            usersNotFound: 'Пользователи не найдены. Уточните поисковой запрос.',
            name: 'Имя',
            email: 'E-mail',
            addGuests: 'Добавить гостей',
            insertUserName: 'Введите имя пользователя или название группы',
            addById: 'Добавить по id',
            invalidEmail: 'Неверный e-mail',
            firstSaveConference: 'Отправка приглашений станет возможна после сохранения конференции',
            enableSendInvitations: 'Автоматически отправить пользователям приглашения при страте конференции'
        },
            ...
            setMax: function (obj, max) {
                ...
                if (obj.plugins.addFromList !== null && obj.plugins.addFromList !== undefined) {
                    if (oldMax !== max) {
                        ...
                        if (max > current) {
                            list.find('.' + obj.plugins.addFromList.cssPrefix + 'iconAdd').addClass("activeIcon");
                        } else {
                            list.find('.' + obj.plugins.addFromList.cssPrefix + 'iconAdd').removeClass("activeIcon");
                        }
                    }
                }
        ...
        containers: {
            ...
            rightPanel: {
                ...
                _render: function (obj) {
                    var container = $('<div style="width: 100%; height: 100%">')
                        .append('<div style="padding-bottom: 6px;">' + obj.locale.addNewParticipants + '</div>');
                    ...
                    if (obj.plugins.addFromList !== null) {
                        inner.append(obj.containers.addFromListPanel.get(obj));
                    }
            ...
            buttonsPanel: {
                ...
                _render: function (obj) {
                    ...
                    for (var i = 0; i < buttons.length; i++) {
                        if (buttons[i] === "inviteGuests" && !obj.participants.guestAccess) {
                            continue;
                        } else if (buttons[i] === "addById" && obj.plugins.addById === null) {
                            continue;
                        } else if (buttons[i] === "addFromList" && obj.plugins.addFromList === null) {
                            continue;
                        }

                        //Создаем кнопку
                        var button = instance._createButton(obj, buttons[i]);
                ...
                _rendered: null,
                /**
                 * Массив с названиями кнопок. Названия должны совпадать с локалью. Перед тем как изменить, смотри
                 * на метод get(obj) там есть всякая хрень, которую надо будет переделывать
                 */
                _buttons: ['addFromList', 'addById', 'inviteGuests'],
                /**
                 ...
                 * @returns {jQuery}
                 */
                _createButton: function (obj, name) {
                    var container = $('<div class="grayButton contentMargin memberListActionButton">');
                    //Добавляем ид
                    container.prop("id", obj.prefix + name);
                    //Получаем локаль
                    var buttonText = obj.locale[name];
                    //Если не найденно - ставим оригинальное имя
                ...
                _applyClick: function (obj, id) {
                    ...
                    if (!obj.containers.hasOwnProperty(id)) {
                        obj.log(obj, "Undefined button : " + id);
                        return;
                    }
                    if (!obj.containers[id].hasOwnProperty("show")) {
                        obj.log(obj, "Invalid button or method : " + id);
                        return;
                    }
                ...
                _render: function (obj) {
                    ...
                    if (ie !== false) {
                        var container = $('<p  style="margin-top: 55px; text-align: center;">'
                            + '<a href="#">'
                            + obj.locale.otherAddingMethods + '</a></p>');
                    } else {
            ...
            memberList: {
                ...
                _render: function (obj) {
                    ...
                    instance._searchField = $('<input tabindex="-1" class="lightGray" maxlength="64" type="text" />');
                    instance._list = $('<div class="usp_members full"></div>');
                    ...
                    instance._usersNotFoundContainer = $('<div style="display: table; width: 100%; height:100%;">' +
                        '<div class="' + obj.cssPrefix + 'notFound">' + obj.locale.usersNotFound + '</div></div>');
                    ...
                    var container = $('<div style="height: 100%; width:100%"></div>')
                        .append('<div style="padding-bottom: 6px;">' + obj.locale.invited + ':</div>');
                    var userList = $('<div class="usp_userList">');
                    var inner = $('<div class="usp_inner">');
                    ...
                    var searchField = this._searchField;
                    //устанавливаем для поля поиска текст по умолчанию
                    searchField.val(obj.locale.insertUserName);
                    ...
                    searchField.focusin(function () {
                        ...
                        if (searchField.val() === obj.locale.insertUserName) {
                    ...
                    searchField.focusout(function () {
                        if (searchField.val().length === 0) {
                            searchField.val(obj.locale.insertUserName);
                            instance._searchString = obj.locale.insertUserName;
                ...
                /**
                 * Осузествляет подстветку найденной подстроки
                 * @param member
                 */
                highlightMember: function (member) {
                    var search_line_value = $('.searchBlock>input').val();
                    var found_line = $('#' + member.htmlId + ' div.mgr_uid');
                    var found_line_value = found_line.text();
                    var html = found_line_value.replace(search_line_value, '<span class="usp_found">' + search_line_value + '</span>');
                    found_line.html(html);
                },
                /**
                ...
                addMember: function (obj, memberContainer, member, params) {
                    ...
                    if (instance._searchString.length !== 0 && instance._searchString !== obj.locale.insertUserName) {
                        //Приводим строку к нижнему регистру
                        var searchString = instance._searchString.toLowerCase();
                        //Приводим имя и айди к нижнему регистру
                        var name = member.displayName.toLowerCase();
                        var id = member.id.toLowerCase();
                        //Если совпадений не найденно,не добавляем пользователя
                        if (name.indexOf(searchString) === -1 && id.indexOf(searchString) === -1) {
                            return false;
                        }
                        //Подсвечиваем поисковую строку
                        instance.highlightMember(member);
                        dbg = instance;
                        $('.usp_notFound').hide();
                    }
                ...
                removeMember: function (obj, id) {
                    ...
                    var participantCount = obj.participants.count(obj);
                    //Обновляем кол-во добавленных пользователей
                    obj.containers.memberList.setUsersCount(obj, participantCount);
                    //Отображаем надпись, если был удалён последний пользователь
            ...
            member: {
                ...
                get: function (obj, member, pluginParams) {
                    ...
                    //Кнопочки, а точнее одна кнопка для удаления пользователя из списка
                    //Если пользователь отключен, то ему не нужны никакие кнопки
                    ...
                    //Клик по контейнеру
                    container.click(function () {
                        obj.onParticipantClick(obj, member, container);
                    });
            ...
            addByIdPanel: {
                ...
                hide: function (obj) {
                    ...
                    obj.log(obj, "hide AddById plugin");
                },
                /**
                 * Показать панель
                 * @param {std_obj} obj
                ...
                show: function (obj) {
                    ...
                    obj.log(obj, "show AddById plugin");
                },
                /**
                 * Контейнер
                 */
            ...
            addFromListPanel: {
                ...
                prepareMember: function (obj, plugin, member, container) {
                    for (var id in member.groups) {
                        if (id === obj.currentGroup) {
                            return false;
                        }
                    }
                ...
                hide: function (obj) {
                    ...
                    obj.log(obj, "hide addFromListPanel plugin");
                ...
                show: function (obj) {
                    ...
                    obj.log(obj, "show addFromListPanel plugin");
                },
                /**
                 * Контейнер
                 */
            ...
            inviteGuestsPanel: {
                ...
                hide: function (obj) {
                    ...
                    obj.log(obj, "hide inviteGuest panel");
                },
                /**
                ...
                show: function (obj) {
                    ...
                    obj.log(obj, "show inviteGuest panel");
                },
                /**
                 * @param {std_obj} obj
                 * @returns {undefined}
            ...
            inviteMessage: {
                ...
                _render: function (obj) {
                    ...
                    //Строка приглашения
                    inviteMessage.text(obj.locale.newParticipantsMessage + ": ");
                ...
                _renderDialog: function (obj) {
                    ...
                    container.dialog("option", "title", obj.locale.titleInviteMessage);
                    ...
                    container.append('<label for="' + obj.prefix + 'inviteMessageSubject" class="smallTitle">'
                        + obj.locale.titleInviteMessageSubject + ':</label>');
                    ...
                    container.append('<label for="' + obj.prefix + 'inviteMessageText" class="smallTitle">' + obj.locale.titleInviteMessage + ':</label>');
                ...
                _renderSaveDialog: function (obj) {
                    ...
                    container.dialog("option", "title", obj.locale.saving);
                    container.append('<div>' + obj.locale.pleaseWait + '</div>');
                ...
                updateDialogBtn: function (obj) {
                    var message = obj.inviteMessage.text;
                    ...
                    obj.containers.inviteMessage._saveXHR =
                        $.post(obj.handlers.saveInviteMessage, {
                            ...
                            .done(function (data) {
                                if (!data.status) {
                                    if (data.error === 'undefined') {
                                        obj.containers.inviteMessage.setError(obj, obj.locale.unknownError);
                                    } else {
                            ...
                            .fail(function (jqXHR, textStatus, errorThrown) {
                                obj.containers.inviteMessage.setError(obj, obj.locale.systemError + ': ' + textStatus);
                            })
                            .always(function () {
            ...
            sendInvitations: {
                ...
                _render: function (obj) {
                    ...
                    container.append('<span> ' + obj.locale.enableSendInvitations + '</span>');
         ...
         * @returns {undefined}
         */
        addParticipant: function (obj, data, params) {
            obj.log(obj, "Add participant");
            obj.log(obj, "Participant data:", data);
            obj.log(obj, "Participant params:", params);
            ...
            if (count > obj.participants.max) {
                obj.log(obj, "Participants list is full, unable to add user");
                return false;
            }
            ...
            count++;
            obj.log(obj, "paricipants.count: " + count);
            ...
            obj.log(obj, "Participants list is full, lock adding buttons");
        ...
        removeParticipant: function (obj, id) {
            ...
            obj.log(obj, "Participants count: " + count);
            ...
            obj.log(obj, "Participants list has free space, unlock adding buttons");
        ...
        setInvitationsStatus: function (obj, status) {
            ...
            switch (status) {
                case $.conferencePlugin.INVITATIONS_STATUSES.NOT_SAVE:
                    obj.containers.inviteButton.disable(obj, obj.locale.firstSaveConference);
                    break;
                case $.conferencePlugin.INVITATIONS_STATUSES.SCHEDULE_NONE:
                    obj.containers.inviteButton.disable(obj, obj.locale.setCorrectSchedule);
                    break;
                    ...
                    buttons: {
                        ...
                        addByIdButton: {
                            ...
                            click: function (plugin, container, button) {
                                //Если кнопка имеет класс disabled, ничего не делаем
                                if (container.hasClass("disabled")) {
                                    return;
                                }
                            ...
                            onSearchFinished: function (plugin, container, searchStatus, searchString, button) {
                                ...
                                //Если кнопка не заблокированна для клика
        ...
        log: function (obj, args) {
            ...
            if (typeof console.debug.apply !== "unknown" && typeof console.debug.apply !== undefined) {
                console.debug.apply(console, data);
            } else {
                Function.prototype.apply.call(console.log, console, data);
            }
        }
    };

    $.fn.usersSelectionPlugin = function (params) {
        var plugin = this.data('usersSelectionPlugin');
        if (plugin === undefined || plugin === false) {
            var plugin = $.extend(true, {}, std_obj);
            $.extend(true, plugin, $.usersSelectionPlugin);
            $.extend(true, plugin, params);
            plugin.prefix = generateUniqueId(plugin.prefix) + "_";
            plugin.parent = this;

            this.append(plugin.getContainer(plugin));
            this.data('usersSelectionPlugin', plugin);
    ...
    $.usersSelectionPlugin = new UsersSelectionPlugin();
